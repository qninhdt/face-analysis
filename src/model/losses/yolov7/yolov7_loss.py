...
class YOLOv7Loss(nn.Module):
    ...
    def __call__(self, inputs, targets):
        ...
        if not self.training:
            ...
            for i in range(self.nl):
                ...
                pred[..., :2] = (pred[..., :2] * 2.0 - 0.5 + grid) * self.strides[i]
                pred[..., 2:4] = (pred[..., 2:4] * 2) ** 2 * self.anchor_grid[
                    i
                ].type_as(pred)
        ...
        for i, prediction in enumerate(inputs):
            ...
            tobj = torch.zeros_like(prediction[..., 0]).type_as(
                prediction
            )  # target obj
            ...
            if n:
                ...
                # age loss
                t = torch.full_like(prediction_pos[:, 5:11], 0).type_as(
                    prediction
                )  # targets
                t[range(n), selected_age] = 1
                age_weight = torch.tensor([1, 2, 2, 4, 4, 4]).type_as(prediction)
                age_loss += F.cross_entropy(
                    prediction_pos[:, 5:11], t, weight=age_weight
                )  # BCE

                # race loss
                t = torch.full_like(prediction_pos[:, 11:14], 0).type_as(
                    prediction
                )  # targets
                t[range(n), selected_race] = 1
                race_weight = torch.tensor([1, 1, 4]).type_as(prediction)
                race_loss += F.cross_entropy(
                    prediction_pos[:, 11:14], t, weight=race_weight
                )  # BCE

                # masked loss
                t = torch.full_like(prediction_pos[:, 14:16], 0).type_as(
                    prediction
                )  # targets
                t[range(n), selected_masked] = 1
                masked_weight = torch.tensor([1, 16]).type_as(prediction)
                masked_loss += F.cross_entropy(
                    prediction_pos[:, 14:16], t, weight=masked_weight
                )  # BCE

                # skintone loss
                t = torch.full_like(prediction_pos[:, 16:20], 0).type_as(
                    prediction
                )  # targets
                t[range(n), selected_skintone] = 1
                skintone_weight = torch.tensor([1, 2, 4, 8]).type_as(prediction)
                skintone_loss += F.cross_entropy(
                    prediction_pos[:, 16:20], t, weight=skintone_weight
                )  # BCE

                # emotion loss
                t = torch.full_like(prediction_pos[:, 20:27], 0).type_as(prediction)
                t[range(n), selected_emotion] = 1
                emotion_weight = torch.tensor([1, 1, 2, 2, 4, 8, 8]).type_as(prediction)
                emotion_loss += F.cross_entropy(
                    prediction_pos[:, 20:27], t, weight=emotion_weight
                )  # BCE

                # gender loss
                t = torch.full_like(prediction_pos[:, 27:29], 0).type_as(prediction)
                t[range(n), selected_gender] = 1
            ...
            # -------------------------------------------#
            obj_loss += (
                F.binary_cross_entropy_with_logits(prediction[..., 4], tobj)
                * self.balance[i]
            )  # obj loss
        # -------------------------------------------#
    ...
    def build_targets(self, predictions, targets):
        ...
        for batch_idx in range(predictions[0].shape[0]):
            ...
            gt_age_per_image = (
                F.one_hot(this_target[:, 5].to(torch.int64), 6)
                .float()
                .unsqueeze(1)
                .repeat(1, pxyxys.shape[0], 1)
            )
            gt_race_per_image = (
                F.one_hot(this_target[:, 6].to(torch.int64), 3)
                .float()
                .unsqueeze(1)
                .repeat(1, pxyxys.shape[0], 1)
            )
            gt_masked_per_image = (
                F.one_hot(this_target[:, 7].to(torch.int64), 2)
                .float()
                .float()
                .unsqueeze(1)
                .repeat(1, pxyxys.shape[0], 1)
            )
            gt_skintone_per_image = (
                F.one_hot(this_target[:, 8].to(torch.int64), 4)
                .float()
                .unsqueeze(1)
                .repeat(1, pxyxys.shape[0], 1)
            )
            gt_emotion_per_image = (
                F.one_hot(this_target[:, 9].to(torch.int64), 7)
                .float()
                .unsqueeze(1)
                .repeat(1, pxyxys.shape[0], 1)
            )
            gt_gender_per_image = (
                F.one_hot(this_target[:, 10].to(torch.int64), 2)
                .float()
                .unsqueeze(1)
                .repeat(1, pxyxys.shape[0], 1)
            )
            ...
            age_preds_ = (
                p_age.float().unsqueeze(0).repeat(num_gt, 1, 1).sigmoid_()
                * p_obj.unsqueeze(0).repeat(num_gt, 1, 1).sigmoid_()
            )
            race_preds_ = (
                p_race.float().unsqueeze(0).repeat(num_gt, 1, 1).sigmoid_()
                * p_obj.unsqueeze(0).repeat(num_gt, 1, 1).sigmoid_()
            )
            masked_preds_ = (
                p_masked.float().unsqueeze(0).repeat(num_gt, 1, 1).sigmoid_()
                * p_obj.unsqueeze(0).repeat(num_gt, 1, 1).sigmoid_()
            )
            skintone_preds_ = (
                p_skintone.float().unsqueeze(0).repeat(num_gt, 1, 1).sigmoid_()
                * p_obj.unsqueeze(0).repeat(num_gt, 1, 1).sigmoid_()
            )
            emotion_preds_ = (
                p_emotion.float().unsqueeze(0).repeat(num_gt, 1, 1).sigmoid_()
                * p_obj.unsqueeze(0).repeat(num_gt, 1, 1).sigmoid_()
            )
            gender_preds_ = (
                p_gender.float().unsqueeze(0).repeat(num_gt, 1, 1).sigmoid_()
                * p_obj.unsqueeze(0).repeat(num_gt, 1, 1).sigmoid_()
            )
            ...
            pair_wise_age_loss = F.binary_cross_entropy_with_logits(
                torch.log(age_y / (1 - age_y)), gt_age_per_image, reduction="none"
            ).sum(-1)
            pair_wise_race_loss = F.binary_cross_entropy_with_logits(
                torch.log(race_y / (1 - race_y)), gt_race_per_image, reduction="none"
            ).sum(-1)
            pair_wise_masked_loss = F.binary_cross_entropy_with_logits(
                torch.log(masked_y / (1 - masked_y)),
                gt_masked_per_image,
                reduction="none",
            ).sum(-1)
            pair_wise_skintone_loss = F.binary_cross_entropy_with_logits(
                torch.log(skintone_y / (1 - skintone_y)),
                gt_skintone_per_image,
                reduction="none",
            ).sum(-1)
            pair_wise_emotion_loss = F.binary_cross_entropy_with_logits(
                torch.log(emotion_y / (1 - emotion_y)),
                gt_emotion_per_image,
                reduction="none",
            ).sum(-1)
            pair_wise_gender_loss = F.binary_cross_entropy_with_logits(
                torch.log(gender_y / (1 - gender_y)),
                gt_gender_per_image,
                reduction="none",
            ).sum(-1)
    ...
    def find_3_positive(self, predictions, targets):
        ...
        for i in range(self.nl):
            ...
            if nt:
                ...
                wh_mask = torch.max(r, 1.0 / r).max(2)[0] < self.threshold  # compare
                t = target[wh_mask]
            ...
            indices.append(
                (b, a, gj.clamp_(0, gain[2] - 1), gi.clamp_(0, gain[1] - 1))
